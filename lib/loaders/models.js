...
module.exports = async (app, modelsDir) => {
  ...
  await requireDir(modelsDir, '.js', (define, name, sourceDir) => {
    moduleName = define[MODULE_NAME] || `${camelCase(name, true)}Model`;
    app[VALIDATE_MODULE_NAME](moduleName);
    models[moduleName] = mongoose.model(moduleName, injectArgs.call(app, sourceDir, getters, define)());
  });
