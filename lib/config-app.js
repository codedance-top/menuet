...
module.exports = async (app, config) => {
  ...
  app
    ...
    .use(bodyParser.raw(config.http.rawParser))
    .use(bodyParser.text(config.http.textParser))
    ...
    app.use((req, res, next) => {
      ...
      if (/^(Mozilla|Opera)\//.test(req.get('user-agent'))) {
        res.set({
          ...
          'Access-Control-Allow-Methods': req.get('access-control-request-method') || (req.method === 'OPTIONS' ? '*' : req.method),
          'Access-Control-Allow-Headers': req.get('access-control-request-headers') || 'Authorization,X-Requested-With,Content-Type'
        });
      }
